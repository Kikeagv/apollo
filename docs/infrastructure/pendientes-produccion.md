...
| 15 | Verificar TLS Full (strict) y baseline WAF en el zone | Hecho 2026-08-18: Configuration Rule `app.usepraxia.com — TLS Full (strict)` (expresión `http.host eq "app.usepraxia.com"`, action SSL = **Strict confirmado visualmente por el fundador**) creada y **Active**; HTTPS 200 y 301 HTTP→HTTPS sin 526 (con tunnel + ingress `http://localhost:80` no se requiere TLS de origen). Managed Rules **no disponibles en Free** (requieren Pro): baseline WAF = rate limit rule `Login + recuperación: 10/min por IP` (expresión sign-in + request-password-reset, **2 req/10 s ≈ 12/min por IP**, Action **Block**, duración 10 s, Status Active, `1/1`). Managed Ruleset + OWASP al pasar a Pro | APO-26 |
...
| 17 | Notificaciones de Coolify por correo (Resend) para deploys/backups fallidos | Hecho 2026-08-19: canal Resend nativo de Coolify (`email_notification_settings`, `resend_enabled`; API key replicada desde el env de la app **sin exponerla**, cifrada en DB; `Praxia <noreply@usepraxia.com>` → `contact@enriqueagv.com`; email de prueba recibido por el fundador). Toggles activos: deploy fallido, backup fallido, tarea programada fallida, docker cleanup fallido, servidor inalcanzable, uso de disco, traefik desactualizado | APO-26 |
| 18 | Health checks y monitoreo externo de `app.usepraxia.com` | Hecho 2026-08-19: health check de recurso Coolify activado (`GET /api/health` → 200, interval 30/s timeout 5 retries 3, se aplica en el próximo deploy/restart); Docker HEALTHCHECK del Dockerfile ya activo (contenedor `Up (healthy)`). Monitoreo externo: tarea programada `praxia-app-health` (cada minuto, dentro del contenedor, contra `https://app.usepraxia.com/api/health` vía Cloudflare+tunnel con marcador anti-spam en transiciones) → notifica por correo Resend si el endpoint deja de responder; primera ejecución success verificada. Nota: monitor alojado en la misma VPS (no detecta caída total del host); opción de servicio externo de uptime para go-live | APO-26 |
...
| 30 | Invitaciones de clínica y de médicos adicionales por **correo real (Resend)** (antes siempre simuladas → el doctor nunca recibía el enlace) | Hecho 2026-08-19 (commit `844fc191`, deploy #9): nuevo seam `clinicInvitationEmailSender()` que selecciona Resend en producción (mismo `IDENTITY_EMAIL_DELIVERY`, guard que prohíbe simulado en prod), asunto `Invitación a <Clínica> en Praxia`, enlace `/activar-invitacion?token=…`; 15 tests nuevos/actualizados, 129 verdes. Tras activar la contraseña, la app **redirige al inicio de sesión a los 3 s** con enlace de respaldo (`4a6e1787`, deploy #10) | APO-15 |
| 31 | Onboarding manual de Clínica documentado y validado en producción | Hecho 2026-08-19: `docs/runbooks/onboarding-clinica-manual.md` (procedimiento SQL + envío Resend + activación + configuración + checklist para 5 clínicas); smoke en prod: invitación real enviada a `contact@enriqueagv.com` (Resend 200), clínica sintética `Clínica Beta Piloto` creada. Gaps: sin página de superadmin (alta por SQL mientras), WhatsApp real (`APO-25`), legal (`APO-5`) | — |
...
| 27 | `www.usepraxia.com` y el apex `usepraxia.com` sirven la landing (decisión del fundador 2026-08-19) | Hecho 2026-08-19: ruta de tunnel `www → localhost:80` + registros DNS tipo Tunnel para `www` y raíz `usepraxia.com` (se eliminó el CNAME heredado `parkingpage.namecheap.com` que causaba 525 y se añadió el registro CNAME de raíz → tunnel, normalizado por CF a tipo Tunnel/Proxied). Landing con botón **"Iniciar sesión" → `https://app.usepraxia.com`** y canonical a www (commit `a01bb5bd`, redeploy OK). Verificado: `www` 200 y `usepraxia.com` 200 (resuelve en 1.1.1.1/8.8.8.8, cert verify 0) | — |
